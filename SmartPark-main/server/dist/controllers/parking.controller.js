...
// ── Entry Logic ──────────────────────────────────────────────
async function handleEntry(vehicle, lotId, res) {
...
// ── Exit Logic ───────────────────────────────────────────────
async function handleExit(vehicle, _lotId, res) {
...
export const getActiveSession = asyncHandler(async (req, res) => {
    ...
    if (!session) {
        return res.status(200).json(new ApiResponse(200, null, "No active session"));
    }
    const now = new Date();
    const durationMs = now.getTime() - session.entryTime.getTime();
    const durationMinutes = Math.ceil(durationMs / (1000 * 60));
    const runningCost = Math.round((durationMinutes / 60) * session.slot.lot.ratePerHour * 100) / 100;
    let path = null;
    const graphData = session.slot.lot.graphData;
