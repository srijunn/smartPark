...
export const createLot = asyncHandler(async (req, res) => {
    ...
    return res.status(201).json(new ApiResponse(201, lot, "Parking lot created"));
});
// ── Update Parking Lot ───────────────────────────────────────
export const updateLot = asyncHandler(async (req, res) => {
    ...
    if (parsed.data.floors !== undefined)
        updateData.floors = parsed.data.floors;
    if (parsed.data.ratePerHour !== undefined)
        updateData.ratePerHour = parsed.data.ratePerHour;
    const lot = await prisma.parkingLot.update({
        where: { id },
        data: updateData,
    });
    return res.status(200).json(new ApiResponse(200, lot, "Lot updated"));
});
// ── Get All Lots ─────────────────────────────────────────────
export const getLots = asyncHandler(async (_req, res) => {
    ...
    return res.status(200).json(new ApiResponse(200, lots.map((lot) => {
...
export const getLot = asyncHandler(async (req, res) => {
    ...
    return res.status(200).json(new ApiResponse(200, lot, "Lot fetched"));
});
