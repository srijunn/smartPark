...
import { findShortestPath, } from "../engines/dijkstra.js";
...
export const createBooking = asyncHandler(async (req, res) => {
    ...
    if (!parsed.success) {
        throw new ApiError(400, "Invalid booking data", parsed.error.issues.map((i) => i.message));
    }
    const { lotId, vehicleId, slotType, preferences, scheduledFor } = parsed.data;
    // Verify lot exists
    ...
    if (existingBooking) {
        throw new ApiError(409, "You already have an active booking. Cancel it before creating a new one.");
    }
    ...
    if (!allocation) {
        throw new ApiError(503, "No available slots matching your preferences. Try different preferences or another lot.");
    }
...
export const getMyBookings = asyncHandler(async (req, res) => {
    ...
    return res.status(200).json(new ApiResponse(200, bookings.map((b) => ({
        id: b.id,
        status: b.status,
...
export const getActiveBooking = asyncHandler(async (req, res) => {
    ...
    if (!booking) {
        ...
        if (sessionGraphData &&
            ...
            sessionPath = findShortestPath(sessionGraphData, activeSession.slot.lot.entryNodeId, activeSession.slot.nodeId);
        }
    ...
    // Compute path
    let path = null;
    const graphData = booking.lot.graphData;
    if (graphData && booking.lot.entryNodeId && booking.slot?.nodeId) {
        path = findShortestPath(graphData, booking.lot.entryNodeId, booking.slot.nodeId);
    }
    return res.status(200).json(new ApiResponse(200, {
        bookingId: booking.id,
        status: booking.status,
