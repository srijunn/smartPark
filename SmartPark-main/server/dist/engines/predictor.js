...
// ── Peak Hours Analysis ──────────────────────────────────────
export async function analyzePeakHours(lotId, daysBack = 30) {
...
export async function predictOccupancy(lotId, targetHour, daysBack = 14) {
    const since = new Date();
    since.setDate(since.getDate() - daysBack);
    ...
    const hourEntries = analytics.filter((e) => e.timestamp.getHours() === targetHour);
...
// ── Revenue Statistics ───────────────────────────────────────
export async function getRevenueStatistics(lotId, daysBack = 30) {
    const since = new Date();
    since.setDate(since.getDate() - daysBack);
...
export async function recordAnalyticsSnapshot(lotId) {
