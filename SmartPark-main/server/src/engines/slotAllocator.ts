...
interface SlotPreferences {
  type?: string;        // REGULAR, EV_CHARGING, HANDICAPPED, PREMIUM
  features?: string[];  // ["shaded", "near_exit"]
}
...
interface AllocationResult {
  ...
  floor: number;
  type: string;
  label: string;        // e.g. "A-202 (Floor 1)"
...
class SlotHeap {
  private heap: { slotId: string; score: number; data: any }[] = [];

  push(slotId: string, score: number, data: any): void {
    this.heap.push({ slotId, score, data });
    this._bubbleUp(this.heap.length - 1);
  }

  pop(): { slotId: string; score: number; data: any } | undefined {
...
export async function allocateBestSlot(
  lotId: string,
  ...
  const graph = lot.graphData as unknown as ParkingGraph | null;
