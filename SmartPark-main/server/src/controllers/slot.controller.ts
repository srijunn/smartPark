...
export const createSlots = asyncHandler(async (req, res) => {
  ...
  return res.status(201).json(
    new ApiResponse(201, { count: created.count, totalSlots }, "Slots created successfully")
  );
});
...
export const updateSlot = asyncHandler(async (req, res) => {
  ...
  if(!id) throw new ApiError(400, "Slot ID is required");
  ...
  return res.status(200).json(
    new ApiResponse(200, updated, "Slot updated successfully")
  );
});
...
export const getOccupancyStats = asyncHandler(async (req, res) => {
  ...
  // Breakdown by type
  const byType: Record<string, { total: number; occupied: number; free: number; reserved: number }> = {};
  for (const s of slots) {
    if (!byType[s.type]) {
      byType[s.type] = { total: 0, occupied: 0, free: 0, reserved: 0 };
    }
    byType[s.type]!.total++;
    if (s.status === "OCCUPIED") byType[s.type]!.occupied++;
    else if (s.status === "RESERVED") byType[s.type]!.reserved++;
    else byType[s.type]!.free++;
  }

  // Breakdown by floor
  const byFloor: Record<number, { total: number; occupied: number; free: number; reserved: number }> = {};
  for (const s of slots) {
    if (!byFloor[s.floor]) {
      byFloor[s.floor] = { total: 0, occupied: 0, free: 0, reserved: 0 };
    }
    byFloor[s.floor]!.total++;
    if (s.status === "OCCUPIED") byFloor[s.floor]!.occupied++;
    else if (s.status === "RESERVED") byFloor[s.floor]!.reserved++;
    else byFloor[s.floor]!.free++;
  }
