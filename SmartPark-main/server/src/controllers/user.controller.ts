...
const getTokens = async(user: { id: string; email: string; name: string }) =>{
    try {
       ...
       try {
            ...
            if (otpError instanceof ApiError && otpError.statusCode === 429) {
                return res.status(403).json(
                    new ApiResponse(403, { requiresVerification: true, userId: user.id, email: user.email },
                    "Account not verified. Please check your email for the existing OTP or wait before requesting a new one.")
                );
            }
            console.error("Failed to send verification OTP:", otpError?.message);
        }
...
const refreshAccessToken = asyncHandler(async (req, res) => {
    ...
    if (!isValid) throw new ApiError(401, "Refresh token is expired or used");
